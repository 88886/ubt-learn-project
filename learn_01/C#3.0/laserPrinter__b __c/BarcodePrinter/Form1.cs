...
namespace LaserPrinter
{
    public partial class ElectronicDataColector : Form
    {
        ...
        IWebSocketConnection sos = null;



        public void webSocket(string tip,string tpo)
        {
            FleckLog.Level = LogLevel.Debug;
            //var allSockets = new List<IWebSocketConnection>();
            server = new WebSocketServer("ws://"+tip+":"+tpo);
            server.Start(socket =>
            {
               
                socket.OnOpen = () =>
                {
                    socket.Send(txtWeight.Text);
