...
//    explicit WorkThread(QObject *parent = 0);
    ...
    QString workArea;//工作区域

    WorkStationThread *workStationThread = NULL; //获取任务线程
    ReadyStationThread *readyStationThread = NULL;//后备区调度AGV线程


    QMap<QTcpSocket*,Agv*> agvMap,socketAgv,socketAgvMap;//初始化存储agv对象，socket对象

    MaterialFlowFrame *frame;//物流调度板块
    ...
    QString startPoint;//发货工位
    QString endPoint;//目标工位
    ...
    ManualTaskPoint *manualTaskPoint;//手动指定工位弹出框
