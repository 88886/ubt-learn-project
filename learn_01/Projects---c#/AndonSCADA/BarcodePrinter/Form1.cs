...
namespace AndonSCADA
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            /*databaseSQL db1 = new databaseSQL();
            db1.SSHConnectMySql();
            String selstr = "select distinct processpoint from list_barcodeprint";

            DataTable reader = db1.Select(selstr);
            if (reader != null)
            {
                for (int i = 0; i < reader.Rows.Count; i++) 
                {
                    comboBox1.Items.Add(reader.Rows[i][0].ToString());
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            {
                flag = false;
                //databaseSQL db2 = new databaseSQL();
                //db2.SSHConnectMySql();
                    ...
                    for (int i = 0; i < reader.Rows.Count; i++)
                    {
                        ...
                        if (reader2 != null && (reader2.Rows.Count == vcount))
                        {
                            for (int j = 0; j < reader2.Rows.Count; j++)
                            {
                                ...
                                //labeldoc.Variables.FreeVariables.Item(itemstr).Value = reader2.Rows[j][0].ToString();
                            }
