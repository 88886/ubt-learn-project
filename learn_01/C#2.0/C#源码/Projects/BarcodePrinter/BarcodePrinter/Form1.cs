...
namespace BarcodePrinter
{
    public partial class Form1 : Form
    {
        ...
        private void button_start_Click(object sender, EventArgs e)
        {
            ...
            textBox_log.Text = "创建Cruzr.exe进程\r\n";
        ...
        private void get_task() 
        {
            ...
            while (true)
            {
                ...
                {
                    flag = false;
                    String selstr = "select distinct tag_index from list_barcodeprint where print_status=0 and processpoint = '" + text + "'";
            ...
            try
            {
                ...
                if (reader != null)
                {
                    ...
                    for (int i = 0; i < reader.Rows.Count; i++)
                    {
                        ...
                        getMessage( "新建打印文档对象\r\n");
                        String selsql1 = "select barcode,varible_index from list_barcodeprint where print_status=0 and tag_index=" + reader.Rows[i][0] + "  and processpoint = '" + text + "' order by varible_index";
                        DataTable reader2 = this.db2.Select(selsql1);
                        getMessage("SN号:"+reader2.Rows[0][0].ToString()+"\r\n");
                        if (reader2 != null && (reader2.Rows.Count == vcount))
                        {
                            ...
                            string text1 = "文档开始打印:标签" + (i + 1).ToString()+"\r\n";
                            getMessage(text1);
                            labeldoc.PrintDocument(); //打印一次
                            string text2 = "文档结束打印:标签" + (i + 1).ToString() + "\r\n";
                            getMessage(text2);
                            labeldoc.FormFeed(); //结束打印

                            String updatesql1 = "update list_barcodeprint set print_status=1 where print_status=0 and tag_index=" + reader.Rows[i][0] + "  and processpoint = '" + text + "'";
