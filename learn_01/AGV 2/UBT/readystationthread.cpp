...
void ReadyStationThread::getReadyStationState(){
    ...
    if(query.next()){
        ...
        sql = "update agv_work_station set state=3 where state=0 and station_name='"+READY_STATION_NAME+"' and  work_area_id = (select id from agv_work_area where name='"+workArea+"')";
        query.exec(sql);
