                ...
                if(a.value()->getState()!="送货中" && a.value()->getState()!="准备送货中" && a.value()->getState()!="故障中"){
                     if(a.value()->getState()=="准备调度中" || a.value()->getState()=="调度中"){
                         if(isContainReadyStation(a.value()->getEndPoint())){
                             ...
                             Station *station = getStation(a.value()->getEndPoint());
                             station->setState(0);
                    ...
                    if(isStart=="是"){
                        ...
                        if(a.value()->getState()=="准备调度中" || a.value()->getState()=="调度中"){
                            if(isContainReadyStation(a.value()->getEndPoint())){
                                ...
                                Station *station = getStation(agv->getEndPoint());
                                station->setState(0);
                        ...
                        if(a.value()->getState()=="返回中" || a.value()->getState()=="准备返回中"){
                            ...
                            getStation(agv->getEndPoint())->setState(0);
...
void WorkThread::initTask(){
    sql = "select * from agv_task where agv_id is NULL and work_area_id in (select id from agv_work_area where name='"+workArea+"')";
    query.exec(sql);
    while(query.next()){
...
void WorkThread::queryStation(QString workArea){
    ...
    while(query.next()){
        ...
        stationStateMap= new QMap<QString,Station*>();
        stationMap.insert(stationName,stationStateMap);
    }

    QMap<QString,QMap<QString,Station*>*>::const_iterator i;
...
bool WorkThread::isContainReadyStation(QString code){
    QMap<QString,QMap<QString,Station*>*>::const_iterator i;
    QMap<QString,Station*>::const_iterator j;

    i = stationMap.find(READY_STATION_NAME);
    bool isContain = false;
    QMap<QString,Station*> *findStationMap = NULL;

    if(i!=stationMap.end()){
        findStationMap = i.value();
        if(findStationMap!=NULL){

            j = findStationMap->find(code);
            if(j!=findStationMap->end()){
                isContain = true;
            }
...
void WorkThread::dispatcherReadyStation(QString endPoint){
    ...
    if(i!=stationMap.end()){
        ...
        if(findStationMap!=NULL){
            ...
            j = findStationMap->find(endPoint);
...
//            }
    ...
    if(i!=stationMap.end()){
        ...
        if(findStationMap!=NULL){
           for(j = findStationMap->constBegin();j!=findStationMap->end();j++){
                if(j.value()->getState()==1){
                    socketAgvMap = getSocketAndAgv(j.value()->getAgvCode());

                    if(socketAgvMap.size()>0){
                        startPoint = j.value()->getCode();
                        Station *station = getStation(endPoint);
                        station->setState(2);
...
//            }
    ...
    if(agvMap.size()!=0){
        ...
        for(a=agvMap.constBegin();a!=agvMap.end();a++){
            ...
            if(a.value()->getState()=="闲置中" && a.key()->state()==QAbstractSocket::ConnectedState && a.value()->getIsDispatcher()==false ){
         ...
         if(a!=agvMap.end()){
             ...
             sql = "update agv_work_station set state = 0 where code='"+endPoint+"'";
             query.exec(sql);
         }
...
void WorkThread::dispatcherStartStation(QString endPoint,QString flowCode){
   ...
   if(stationMap.size()>0){
       ...
       for(i=stationMap.constBegin();i!=stationMap.end();i++){
           ...
           for(j = i.value()->constBegin(); j!=i.value()->end();j++){
               if(j.value()->getCode()==endPoint){
                   x = j.value()->getX();
                   y = j.value()->getY();
                   break;
               }
        ...
        if(a!=agvMap.end()){
            ...
            if(i!=stationMap.end()){
                ...
                if(findStationMap!=NULL){
                    ...
                    for(j = findStationMap->constBegin();j!=findStationMap->end();j++){
                        if(a.key()->objectName() == j.value()->getAgvCode()){
                            a.value()->setStartPoint(j.value()->getCode());
                            break;
                        }
                    }
...
//                    }
            ...
            //调不到车时
            sql = "update agv_task set task_state='等待中' where flow_code='"+flowCode+"'";
            query.exec(sql);
        }
                    ...
                    if(!isContainReadyStation(a.value()->getEndPoint())){
                        ...
                        if(a.value()->getSendData().startsWith(BACKTOSTATION)){
                            ...
                            for(i = stationMap.constBegin();i!=stationMap.constEnd();i++){
                                findStationMap = i.value();
                                if(findStationMap!=NULL){
                                    Station *station = findStationMap->find(a.value()->getStartPoint()).value();
                                    station->setAgvCode(NULL);
                                    station->setState(0);
                                }
                            }
                        ...
                        query.exec(sql);
                          ...
                          findStationMap = stationMap.find(READY_STATION_NAME).value();
                          findStationMap->find(agv->getEndPoint()).value()->setState(0);
...
Station* WorkThread::getStation(QString stationCode){
    QMap<QString,Station*> *findStationMap = NULL;
    QMap<QString,Station*>::const_iterator j;
    ...
    for(i = stationMap.constBegin();i!=stationMap.constEnd();i++){
        findStationMap = i.value();
        if(findStationMap!=NULL){
            station = findStationMap->find(stationCode).value();
            break;
        }
    }
    ...
    if(read=="TRANSACTION|RSUCCESS"){
       ...
       if(agv->getReSendTimer()!=NULL){
        ...
        if(agv->getState()=="返回中"){
            ...
            Station *station = getStation(agv->getStartPoint());
            station->setState(0);
...
//        }
        ...
        if(agv->getNGTimes()<ngTimes){
            ...
            if(isContainReadyStation(agv->getEndPoint())){
                if(agv->getState()=="准备调度中"){
                    ...
                    getStation(agv->getStartPoint())->setAgvCode(NULL);
                    getStation(agv->getStartPoint())->setState(0);
    ...
    if(read=="TRANSACTION|OK"){
        ...
        Station *station = getStation(agv->getEndPoint());
        station->setState(1);
        station->setAgvCode(agv->getCode());
...
void WorkThread::navigation(QTcpSocket *socket, Agv *agv,QString read){
   ...
   //机器送货NG指令
    ...
    if(read=="NAVIGATION|OK"){
        ...
        for(j = i.value()->constBegin();j!=i.value()->end();j++){
            if(j.value()->getState() == 0){
                message = BACKTOSTATION+j.value()->getCode();
                agv->setStartPoint(agv->getEndPoint());
                agv->setEndPoint(j.value()->getCode());
...
void WorkThread::getWorkStationLocation(Agv *agv, QString read){
     ...
     if(patternStationLocation.indexIn(read)!=-1){
         ...
         Station *station = getStation(agv->getStationName());
         station->setX(x);
         station->setY(y);
